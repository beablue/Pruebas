...
# TO THINK ABOUT IT
 # When we recall the caracter placed in an exact position in an array, the Index
 # works as usual. But when we use the tipe of method slice
 # (or the shortcut [start,interval length]), the coordenates of start don't work as
 # the usual Index, it separate each part of the array, including the brackets.
 # So, if we request array[4,0] we still are in the array (the bracket) and we
